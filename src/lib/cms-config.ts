...
export const cmsImages: CmsImages = getLocalStorage("ss_cms_images", DEFAULT_IMAGES);
export const cmsTexts: CmsTexts = getLocalStorage("ss_cms_texts", DEFAULT_TEXTS);
export const cmsFaqs: FaqCategory[] = getLocalStorage("ss_cms_faqs", DEFAULT_FAQS);

export function saveCmsConfig(images: CmsImages, texts: CmsTexts, faqs: FaqCategory[]) {
  if (typeof window === "undefined") return;
  localStorage.setItem("ss_cms_images", JSON.stringify(images));
  localStorage.setItem("ss_cms_texts", JSON.stringify(texts));
  localStorage.setItem("ss_cms_faqs", JSON.stringify(faqs));
}
